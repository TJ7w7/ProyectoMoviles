...
class DetalleAsignacion : Fragment() {
    ...
    private val puntosConIncidencia = mutableSetOf<String>()
    ...
    private fun cargarIncidencias() {
        // Escuchamos en tiempo real si llegan reportes
        db.collection("incidencias")
            .whereEqualTo("asignacionId", asignacionId)
            .addSnapshotListener { snapshots, e ->
                if (e != null) return@addSnapshotListener

                puntosConIncidencia.clear()

                for (doc in snapshots!!) {
                    // Si la incidencia no está resuelta, agregamos el ID del punto a la lista roja
                    val estado = doc.getString("estado")
                    if (estado != "Resuelta") {
                        val puntoId = doc.getString("puntoId")
                        if (puntoId != null) {
                            puntosConIncidencia.add(puntoId)
                        }
                    }
                }

                // Refrescamos la lista para que se pinten de rojo
    ...
    private fun setupRecyclerViews() {
        ...
        puntosAdapter = VerPuntosAdapter(puntosRuta, puntosConIncidencia){ punto ->
            // Cuando el admin toca un punto, abrimos el diálogo de gestión
            mostrarDialogoGestionarPunto(punto)
        }
    ...
    // --- NUEVAS FUNCIONES PARA GESTIONAR INCIDENCIAS ---
    private fun mostrarDialogoGestionarPunto(punto: VerPuntosRuta.PuntoConOrden) {
        // Verificamos si realmente tiene incidencia
        val tieneIncidencia = puntosConIncidencia.contains(punto.punto.id)

        val titulo = if (tieneIncidencia) "🚨 ATENCIÓN: INCIDENCIA REPORTADA" else "Gestionar Punto"
        val mensaje = if (tieneIncidencia)
            "El recolector reportó un problema en el Punto ${punto.orden}: ${punto.punto.nombre}.\n\n¿Deseas VALIDAR este punto manualmente para desbloquear la ruta?"
        else
            "Este punto NO tiene incidencias reportadas.\n\n¿Estás seguro que deseas forzar su validación? Esto podría alterar la ruta."
        ...
        builder.setPositiveButton("✅ VALIDAR / SALTAR") { _, _ ->
            validarPuntoManualmente(punto)
            // Opcional: Marcar incidencia como resuelta en BD
        }
        builder.setNegativeButton("Cancelar", null)
        builder.show()
    }

    private fun validarPuntoManualmente(punto: VerPuntosRuta.PuntoConOrden) {
        // 1. Preparamos una evidencia "ficticia" de tipo administrativo
        val evidenciaAdmin = hashMapOf(
            "asignacionId" to asignacionId,
            "puntoId" to punto.punto.id,
            "orden" to punto.orden,
            "fechaRecoleccion" to System.currentTimeMillis(),
            "fotos" to emptyList<String>(), // Lista vacía, no hay fotos
            "tipo" to "ADMIN_OMITIDO", // Marca especial para saber que fue el admin
            "observacion" to "Punto validado manualmente por el Administrador (Incidencia)",
            "recolectorId" to "ADMINISTRADOR"
        )

        // 2. Guardamos en la colección que el Recolector está vigilando
        db.collection("evidencias_recoleccion")
            .add(evidenciaAdmin)
            .addOnSuccessListener {
                Toast.makeText(requireContext(), "✅ Punto desbloqueado exitosamente", Toast.LENGTH_SHORT).show()

                // Opcional: Recargar la vista para ver el cambio (aunque no afecte visualmente al admin mucho)
                cargarEvidencias()
            }
            .addOnFailureListener { e ->
                Toast.makeText(requireContext(), "Error al validar: ${e.message}", Toast.LENGTH_SHORT).show()
            }
    }
